...
想定ペルソナ: **広野辰巳（30歳・IT エンジニア）** — モノトーン、シック、ブルックリンスタイルを好む。洋画と酒が趣味。ツールには利便性とデザイン性の両方を求め、**安っぽいアイコンを嫌う**。

- **モノトーン基調。** 配色は `zinc` 系の無彩色で統一する。鮮やかなアクセントカラー（青ボタン等）は使わない。主ボタンはライトで `bg-zinc-900`、ダークで `bg-zinc-100` の反転スタイル。
- **絵文字をアイコンとして使わない。** アイコンは `src/components/icons.tsx` のラインアイコン（SVG・stroke ベース）を使う。足りなければ同ファイルに追加する（Lucide 風、24px viewBox・stroke-width 2）。
- **機能上の色は例外。** マスクの赤オーバーレイやエラーメッセージの赤など、意味を持つ色は残す。
- タイポグラフィは控えめに引き締める（見出しの `tracking`、数値の `tabular-nums` など）。装飾よりも余白と階層で見せる。
